...
byte midiChannel=2;//channel 2 by default
byte newMidiChannel=0;//temp beat length for manual entry
byte midiTrigVal=0;//to store drum trigger state to push on the shift register
...
void setup(){
  ...
  //detect if 'B' (BANK) button is pressed during startup, set to midiMode----
  pinMode(2,OUTPUT);
  digitalWrite(2,LOW);
  pinMode(A3,INPUT_PULLUP);
  bool in=digitalRead(A3);
  if(in==LOW)
    mode=midiMode; //set to midiMode
  pinMode(2,INPUT);

  //MIDI config stuff----
  MIDI.begin(MIDI_CHANNEL_OMNI);              // Launch MIDI with default option
  Serial.begin(115200);  //override the default MIDI baud-rate- comment this line if you plan to use it with commercial MIDI gear
  MIDI.setHandleNoteOn(midiOn);
  MIDI.setHandleNoteOff(midiOff);

  //setup timer1 for internal clock and BPM calculation----
  ...
  switch (customKey){
    ...
    case '1':
      ...
      if (mode==home)
        curChannel=0;
        if (mode==midiMode)
        newMidiChannel=(newMidiChannel*10)+1;
      break;
     case '2':
      if (mode==editNotes){
        if(!altBank)
       ...
       if (mode==home)
        curChannel=1;
        if (mode==midiMode)
        newMidiChannel=(newMidiChannel*10)+2;
      break;
     case '3':
      if (mode==editNotes){
        if(!altBank)
       ...
       if (mode==home)
        curChannel=2;
        if (mode==midiMode)
        newMidiChannel=(newMidiChannel*10)+3;
      break;
     case '4':
        if (mode==editNotes){
        if(!altBank)
       ...
       if (mode==home)
        curChannel=3;
        if (mode==midiMode)
        newMidiChannel=(newMidiChannel*10)+4;
      break;
     case '5':
      if (mode==editNotes){
        if(!altBank)
      ...
      if (mode==home)
        curChannel=4;
        if (mode==midiMode)
        newMidiChannel=(newMidiChannel*10)+5;
      break;
     case '6':
      if (mode==editNotes){
        if(!altBank)
      ...
      if (mode==home)
        curChannel=5;
        if (mode==midiMode)
        newMidiChannel=(newMidiChannel*10)+6;
      break;
     case '7':
      if (mode==editNotes){
        if(!altBank)
      ...
      if (mode==home)
        curChannel=6;
        if (mode==midiMode)
        newMidiChannel=(newMidiChannel*10)+7;
      break;
     case '8':
      if (mode==editNotes){
        if(!altBank)
      ...
      if (mode==home)
        curChannel=7;
        if (mode==midiMode)
        newMidiChannel=(newMidiChannel*10)+8;
      break;
     case '9':
      if (mode==editNotes){
        if(!altBank)
      ...
      if (mode==midiMode)
        newMidiChannel=(newMidiChannel*10)+9;
      break;
     case '*':
      if (mode==editNotes){
        if(!altBank)
     ...
     case '0':
      ...
      if (mode==midiMode)
        newMidiChannel=(newMidiChannel*10)+0;
     ...
     case '+':
      ...
      if((mode==home || mode==midiMode) && screenBrightness<maxScreenBrightness)
        screenBrightness+=1;
      break;
     case '-':
     ...
     if (mode==editNotes && beatnotes>12)
        beatnotes-=12;
     if((mode==home || mode==midiMode) && screenBrightness>0)
        screenBrightness-=1;
      break;
     case '#':
      if (mode==editNotes){
    ...
    if (mode==editBeatLen)
     ...
     if(mode==midiMode) //display midi channel number
     {for(byte i=0;i<5;i++)
         lc.setRow(0,i+3,(cCD[i]&0x0f) | (cGH[i]&0xf0));
      displayNum(1,midiChannel,true);
     } 
  }
  //detect button press
...
void midiOn(byte chn,byte note,byte vel){
  note=note%12;
  if(chn== midiChannel and note<8 and note>=0)
    {midiTrigVal=midiTrigVal | 1<<note;
    shiftOutTrig();
      }
  }
...
//shift out function only for MIDI triggers----
  void shiftOutTrig(){
    for(byte i=0;i<8;i++)
      { 
        //set data bit:
